  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="bg-app-card rounded-[20px] shadow-2xl overflow-hidden border border-app-border">
           ...
           <div className="h-[450px] relative">
              ...
              <div className="absolute top-4 left-4 right-4 flex flex-col sm:flex-row justify-between items-start gap-4">
                 <div className="bg-black/40 backdrop-blur-xl p-4 md:p-6 rounded-[20px] border border-white/10 shadow-2xl">
                 ...
                 <div className="bg-black/40 backdrop-blur-xl p-4 md:p-6 rounded-[20px] border border-white/10 shadow-2xl text-right sm:ml-auto">
           ...
           <div className="p-6 md:p-16">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12 md:mb-16 px-2 md:px-4">
                 <div className="flex items-center gap-4 md:gap-6">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-app-card rounded-[20px] overflow-hidden border border-app-border">
                    ...
                    <div>
                       <p className="text-[10px] md:text-xs font-bold text-app-text/30 uppercase tracking-widest mb-1">Your Delivery Partner</p>
                       <h3 className="text-xl md:text-2xl font-bold text-app-text">Said Al-Mabsali</h3>
                 ...
                 <div className="flex gap-4 w-full md:w-auto">
                    <button className="flex-1 md:flex-none justify-center bg-app-card p-4 rounded-[20px] hover:bg-app-card/80 transition-colors border border-app-border group flex items-center">
                       <Phone className="w-6 h-6 text-app-text group-hover:text-primary transition-colors" />
                    </button>
                    <button className="flex-[2] md:flex-none bg-primary text-white px-10 py-4 md:py-0 rounded-[20px] font-black uppercase tracking-tighter hover:bg-primary-dark transition-all shadow-lg shadow-primary/20">
                       Chat
                    </button>
                 </div>
              </div>
              ...
              {/* Progress Timeline */}
              <div className="space-y-12 relative before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-app-border">
                 {steps.map((s) => (
                    <div key={s.id} className="relative pl-12">
                       <div className={`absolute left-0 top-1 w-10 h-10 rounded-[20px] flex items-center justify-center transition-all ${step >= s.id ? 'bg-primary text-white scale-110 shadow-lg shadow-primary/30' : 'bg-app-card border border-app-border text-app-text/10'}`}>
                       ...
                       <div className="flex justify-between items-start">
                          <div>
                             <h4 className={`text-xl font-bold transition-all uppercase italic tracking-tighter ${step >= s.id ? 'text-app-text' : 'text-app-text/20'}`}>{s.title}</h4>
                             <p className={`text-sm mt-1 transition-all ${step >= s.id ? 'text-app-text/40' : 'text-app-text/10'}`}>{s.description}</p>
                          </div>
                          <span className={`text-sm font-bold font-display ${step >= s.id ? 'text-app-text/30' : 'text-app-text/10'}`}>{s.time}</span>
                       </div>
                    </div>
                 ))}
              </div>

              <div className="mt-16 p-10 bg-app-text/[0.02] rounded-[20px] border border-app-border text-center relative overflow-hidden group">
                 ...
                 <div className="relative z-10">
                    ...
                    <p className="text-app-text/30 text-sm leading-relaxed max-w-md mx-auto italic">Open the DroneExpress App on your phone to see the drone approaching in Augmented Reality.</p>
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="mt-12 flex justify-between items-center px-10">
           <Link to="/restaurants" className="font-bold text-app-text/20 hover:text-primary transition-colors uppercase tracking-widest text-[10px]">Order Something Else</Link>
